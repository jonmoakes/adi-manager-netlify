...
import { pupils, deleteConfirmedMessage } from "../../../strings/strings";
...
const useDeletePupilEntry = () => {
  ...
  const deletePupilEntry = async () => {
    ...
    try {
      if (!userSnapshot.exists || !entry || category !== pupils) return;
      ...
      await updateDoc(userRef, { pupilsEntries: arrayRemove(entryToRemove) });

...
import usePupilsEntriesSnapshotListener from "./use-pupils-entries-snapshot-listener";
...
import { deleteEntryPath, editPupilEntryPath } from "../../../strings/strings";

const PupTable = () => {
  usePupilsEntriesSnapshotListener();

  const errorMessage = useSelector(selectErrorMessage);
  let pupilsEntries = useSelector(selectPupilsEntries);
  const isLoading = useSelector(selectIsLoading);

  const columns = useMemo(() => COLUMNS, []);
  const data = useMemo(() => pupilsEntries, [pupilsEntries]);
  ...
  pupilsEntries = chosenEntry;
  const dataLength = data.length;
  const rowsLength = rows.length;
  const chosenEntryLength = chosenEntry.length;
  const entries = pupilsEntries;
  const editPath = editPupilEntryPath;

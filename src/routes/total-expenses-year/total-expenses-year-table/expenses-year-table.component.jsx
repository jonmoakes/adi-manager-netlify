...
import useTotalExpensesYearEntriesSnapshotListener from "./use-total-expenses-year-entries-snapshot-listener";
...
const ExpensesYearTable = () => {
  useTotalExpensesYearEntriesSnapshotListener();
  ...
  totalExpensesYearEntries = chosenEntry;
  const dataLength = data.length;
  const rowsLength = rows.length;
  const chosenEntryLength = chosenEntry.length;
  const entries = totalExpensesYearEntries;
  const editPath = editTotalExpensesYearEntryPath;
  const deletePath = deleteEntryPath;

  const getSelectedAmounts = totalExpensesYearEntries.map((entry) => {
    return Number(entry.yearlyAmount);
  });
...
export default ExpensesYearTable;

...
import usePupilProgressEntriesSnapshotListener from "./use-pupil-progress-entries-snapshot-listener";
...
const ProgTable = () => {
  usePupilProgressEntriesSnapshotListener();

  const errorMessage = useSelector(selectErrorMessage);
  let pupilProgressEntries = useSelector(selectPupilProgressEntries);
  const isLoading = useSelector(selectIsLoading);

  const columns = useMemo(() => COLUMNS, []);
  const data = useMemo(() => pupilProgressEntries, [pupilProgressEntries]);

  const initialState = useMemo(
    () => ({ sortBy: [{ id: "name", desc: false }], pageSize: 25 }),
    []
  );
  ...
  pupilProgressEntries = chosenEntry;
  const dataLength = data.length;
  const rowsLength = rows.length;
  const chosenEntryLength = chosenEntry.length;
  const entries = pupilProgressEntries;
  const editPath = editPupilProgressEntryPath;
